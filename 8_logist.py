...
# راح تنزل 2 ملفات واحد البيانات و الثاني داخلة اسماء الاعمدة ".data" ممكن تنزلها و اذا كانت 
...
# تحويل البيانات لتكون جاهزة للتدريب و تحديد الخوارمية
# "ColumnTransformer" اريد عمل تحويل 
# "OneHotEncoder" من عناصر كتجري و حافظ على البيانات بس حولها بستخدام 'cat'
# (handle_unknown='ignore'), x.columns)  غير مفهوم
preprocessor = ColumnTransformer([
    ('cat', OneHotEncoder(handle_unknown='ignore'), x.columns) # ابحث عنها ممكن تعمل كماها اختصار باشياء ثانية
])
model = make_pipeline(preprocessor, LogisticRegression(max_iter=1000))  # ابحث max_iter=1000
...
# ConfusionMatrixDisplay(cm, display_labels=model.named_steps['logisticregression'].classes_).plot(cmap='Blues')
